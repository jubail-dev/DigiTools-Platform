...
const SelectedProductCart = ({
  ...
  const totalPrice = selected.reduce(
    (acc, value) => acc + value.price.amount,
    0,
  );

  const handleRemoveButton = (product: ProductType) => {
    const restProducts = selected.filter(
      (selectedProduct) => selectedProduct.id !== product.id,
    );
    setSelected(restProducts);
    toast.success(`Succesfully Remove ${product.title}`, {
      position: "top-center",
      autoClose: 2500,
  ...
  const handleDelleteAllButton = () => {
    setSelected([]);
    toast.success(`Delete All Cards`, {
      position: "top-center",
      autoClose: 2500,
      hideProgressBar: false,
      closeOnClick: false,
  ...
  return (
    <>
      <div className="w-screen container mx-auto py-10">
        ...
        <button
          className="bg-blue-500 py-3 rounded-full text-center text-white font-bold text-xl w-screen container mx-auto"
          onClick={() => handleDelleteAllButton()}
        >
          Delete All
        </button>
      </div>
